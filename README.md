...
- Start every normal game with a fresh six-question pack generated by Qwen.
- Submit a private position and confidence rating.
- Encounter occasional, deterministically selected round modifiers.
- Give every question its own AI-written discussion prompt and surprise angle.
...
Room state is stored as versioned snapshots: SQLite for local development and Postgres for hosted deployments. Each participant receives an unguessable reconnect credential; only its hash is stored by the server, and game actions derive the acting player from that credential. At game start, FastAPI asks the authenticated Oracle gateway for six original questions and their round-specific discussion copy. When the final round closes, Qwen selects and describes the most interesting results from a server-calculated fact catalog. The deterministic engine still owns the modifier schedule, targets, instructions, arithmetic, and scoring.
...
validation retry malformed generations. Curated questions are used only when the AI
